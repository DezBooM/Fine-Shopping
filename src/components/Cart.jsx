...
function Cart() {
  ...
  return (
    <div className="flex flex-col sm:flex-row w-full justify-between">
        ...
        {cart.map((product) => (
          ...
          >
            <img
              ...
              alt={product.name}
            />
            <h1 className="font-bold text-sm sm:text-base ml-2">
              {product.name}
            </h1>
            <span className="text-xs sm:text-base">${product.price}</span>
      ...
      <div className="flex flex-col relative items-center justify-start gap-10 font-bold bg-lime-300 dark:bg-stone-900  w-full sm:w-1/3 pb-20 sm:px-4 sm:pt-10 mt-5 sm:mt-0 text-lg rounded-b-lg">
        <h1 className="text-2xl">Subtotal({cart.length}) items</h1>
        <span className="text-lg">Total: ${total}</span>
        <button
          onClick={handlePurchase}
          className="bg-lime-900 text-stone-100 dark:bg-stone-600 px-4 py-2 rounded-full"
        >
